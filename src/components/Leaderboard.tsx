...
export default function Leaderboard({ entries, currentTrackId }: LeaderboardProps) {
  ...
  return (
    <div id="leaderboard_comp" className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl text-white">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Award className="w-6 h-6 text-yellow-500 animate-pulse" />
          <h2 className="text-xl font-bold font-sans tracking-tight">Circuit Leaderboard</h2>
        </div>
        <span className="text-xs font-mono text-slate-400 bg-slate-800 px-3 py-1 rounded-full uppercase">
      ...
      {filteredEntries.length === 0 ? (
        <div className="text-center py-8 text-slate-500">
          <p className="font-mono text-sm">No recorded telemetry. Be the first to establish a lap record!</p>
        </div>
      ) : (
          ...
          {filteredEntries.map((entry, index) => {
            ...
            return (
              <div
                ...
                key={entry.id}
                className={`flex items-center justify-between p-3 rounded-lg border text-sm transition-all duration-300 hover:scale-[1.01] ${
                  isPodium
                    ? podiumColors[index]
              ...
              >
                ...
                <div className="flex items-center gap-6">
                  ...
                  {/* Avg Speed */}
                  <div className="hidden sm:flex items-center gap-1 min-w-[75px] text-right justify-end text-xs text-slate-400 font-mono">
                    <Activity className="w-3.5 h-3.5 text-emerald-500/80" />
                    <span>{entry.avgSpeed} <span className="text-[10px]">km/h</span></span>
                  </div>

                  {/* Max G */}
                  <div className="hidden sm:flex items-center gap-1 min-w-[55px] text-right justify-end text-xs text-slate-400 font-mono">
